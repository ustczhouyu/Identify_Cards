...
def fix_birthplace(birthplace, idnum, df_district, areacode_unit_address):
    ...
    if district in list(df_district['name']):
        code = df_district.loc[df_district.name == district, 'code'].tolist()[0]
        code = str(code)
        return areacode_unit_address[code]['address']
...
if __name__ == '__main__':
    ...
    # df_provinces = pd.read_csv(r'./data_correction/data/provinces.csv')
    df_district  = pd.read_csv('./data_correction/data/district.csv')
    district = fix_birthplace('钟祥市', '420881199102164446', df_district, areacode_unit_address)
